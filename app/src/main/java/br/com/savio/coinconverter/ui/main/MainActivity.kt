...
class MainActivity : AppCompatActivity() {
    ...
    private fun bindListeners() {
        binding.tilValue.editText?.doAfterTextChanged {
            binding.btnInverter.isEnabled = it != null && it.toString().isNotEmpty()
            binding.btnConverter.isEnabled = it != null && it.toString().isNotEmpty()
            binding.btnSave.isEnabled = false
        }

        binding.btnInverter.setOnClickListener {
            it.hideSoftKeyboard();
            var swap = binding.tvTo.text
            binding.tvTo.setText(binding.tvFrom.text, false)
            binding.tvFrom.setText(swap, false)
        }
